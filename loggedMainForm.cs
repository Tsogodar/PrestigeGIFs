...
namespace PG2
{
    public partial class loggedMainForm : Form
    {
        string loggedUser;
        public loggedMainForm(string user)
        {
            InitializeComponent();
        ...
        private void loggedMainForm_Load(object sender, EventArgs e)
        {
        ...
        private void uploadButton_Click(object sender, EventArgs e)
        {
            ...
            {
                Filter = "Pliki (JPG,PNG,GIF)|*.JPG;*.PNG;*.GIF",
                Multiselect = false
            };
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                var image = Image.FromFile(dialog.FileName);
                var uploadFile = new newFile(image, dialog.SafeFileName,loggedUser);
                uploadFile.Show();
                this.Close();
            }
        }
